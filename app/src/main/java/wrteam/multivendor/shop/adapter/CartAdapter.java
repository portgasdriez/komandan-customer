...
public class CartAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(@NotNull RecyclerView.ViewHolder holderParent, final int position) {
        try {
            ...
            if (holderParent instanceof HolderItems) {
                ...
                holder.btnAddQuantity.setOnClickListener(view -> addQuantity(cart, cart.getItems().get(0), holder, true, maxCartCont));

                holder.btnMinusQuantity.setOnClickListener(view -> addQuantity(cart, cart.getItems().get(0), holder, false, maxCartCont));
    ...
    @SuppressLint("SetTextI18n")
    public void addQuantity(Cart cart, CartItems cartItem, HolderItems holder, boolean isAdd, String maxCartCont) {
        try {
